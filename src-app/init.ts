...
app.on('ready', () => {
    createWindow('hud', config.screenWidth, config.screenHeight, config.windowX, config.windowY, config.frameless)
    if (config.showDexNav) {
        createWindow('dexnav', config.dexNavWidth, config.dexNavHeight, config.dexNavX || -1, config.dexNavY || -1, config.frameless)
    }
});
