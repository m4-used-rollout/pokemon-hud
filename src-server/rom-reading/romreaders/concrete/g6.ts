...
namespace RomReader {
    ...
    export class Gen6 extends Generic {
        ...
        CollapseSeenForms(seen: number[]) {
            seen.filter(s => s > 721).map(s => Object.keys(formSeenMap).filter(k => formSeenMap[parseInt(k)].some(e => e == s)).map(parseInt).pop() || s).forEach(s => seen.indexOf(s) < 0 && seen.push(s));
            return seen.filter(s => s <= 721);
        }
    }
}