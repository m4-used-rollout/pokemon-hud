...
namespace RomReader {
    ...
    export abstract class NDSReader extends RomReaderBase {
        ...
        protected readEvolutions(evoNarc: Tools.NARChive) {
            evoNarc.files.forEach((file, i) => this.GetSpeciesById(i).evolutions = this.ReadArray(file, 0, 6, 7).map((data): Pokemon.Evolution => {
                ...
                return this.ParseEvolution(method, evoParam, speciesId);
            }).filter(e => !!e));
        }
