    ...
    class TrainerTracker extends Tracker<KnownActions> {
        ...
        public Analyzer(newState: TPP.RunStatus, oldState: TPP.RunStatus, dispatch: (action: KnownActions) => void): void {
            ...
            if (newState.in_battle && !oldState.in_battle
                ...
                dispatch({
                    ...
                    id: newState.enemy_trainers[0].id || newState.enemy_trainers[1] && newState.enemy_trainers[1].id,
                    classId: newState.enemy_trainers[0].class_id || newState.enemy_trainers[1] && newState.enemy_trainers[1].class_id,
                    className: newState.enemy_trainers[0].class_name || newState.enemy_trainers[1] && newState.enemy_trainers[1].class_name,
        ...
        public Reporter(state: TPP.RunStatus): TPP.RunStatus {
            ...
            function isGoalTrainer(id: number, classId: number) {
                return goalTrainers.some(t => (t.id == id && (!t.classId || t.classId == classId)) || (!t.id && t.classId == classId));
            }
            ...
            if (trainerGoals) {
                ...
                if (!trainerGoals.requiredTrainerIds && trainerGoals.requiredTrainerClasses)
                    trainerGoals.requiredTrainerClasses.forEach(classId => goalTrainers.push({ classId }));
            }
